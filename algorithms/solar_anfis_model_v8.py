...
# -*- coding: utf-8 -*-
"""
solar_anfis_model_v8.py
=======================
v8 — Hybrid ANFIS + POA Prior + Panel Calibration

相較 v5 增加 1 個特徵：
  - panel_calib：每片 panel 的長期平均 PR_norm（calibration factor）

動機：資料診斷發現 A/B 兩片同條件下 PR 差異 std≈12%，主要來自
14 個不同 MPPT 控制器的特性差異。直接給模型每片的「長期 calibration」
讓它學「這片硬體的個性偏差」，預期 overall R² 提升、Top-1 Acc 改善。

注意：panel_calib 是用全體資料平均（含 test 時段），這是合理的「長期特性」
而非 lookup leak。實際部署時，新追日 panel 用初期幾週資料估算 calib 即可
...
def build_anfis_model(input_dim, num_mfs=7):
    """ANFIS v8：sigmoid 輸出（PR_norm ∈ [0, 1]，物理合理）"""
...
# ════════════════════════════════════════════════════════════════
def create_features_v8(df, poa_min: float = 50.0):
    """
    target = PR_norm = power_W / (theoretical_poa × area × η_STC)
    ...
    # v8 新增：panel_calib = 該片 panel 的長期平均 PR_norm
    if 'panel_id' in df.columns:
        panel_avg = df.groupby('panel_id')['PR_norm'].transform('mean')
        df['panel_calib'] = panel_avg
        n_panels = df['panel_id'].nunique()
        calib_stats = df.groupby('panel_id')['panel_calib'].first()
        print(f"  Panel calibration: {n_panels} panels, calib 範圍 {calib_stats.min():.4f}–{calib_stats.max():.4f}")
        print(f"    P10–P90: {calib_stats.quantile(0.1):.4f}–{calib_stats.quantile(0.9):.4f}, mean {calib_stats.mean():.4f}")
    else:
        print("  ⚠ 缺少 panel_id，panel_calib 設為 0.4 (全資料集平均)")
        df['panel_calib'] = 0.4
...
def evaluate_ranking_mode_b(model, scaler_X, df_test_meta: pd.DataFrame,
    ...
    # v8: Mode B 對 OOD 角度推論，用所有 panel 的平均 calib（無特定 panel）
    default_calib = 0.4  # 全資料集平均，可從 model_config 載入更精準值
    panel_calib_tile = np.full(len(hour_sin), default_calib, dtype='float32')

    X_big = np.column_stack([hour_sin, hour_cos, day_sin, day_cos,
                              tilt_sin, tilt_cos, azi_sin, azi_cos,
                              clearness_tile,
                              panel_calib_tile]).astype('float32')
...
def main(file_path=None, output_dir=None, poa_min: float = 50.0,
    ...
    print(f"=== ANFIS v8 訓練啟動 (Hybrid POA + Panel Calibration) ===")
    print(f"  target = PR_norm = power_W / (theoretical_poa × {PANEL_AREA_M2} × {PANEL_EFF_STC})")
    ...
    # ── 特徵工程
    result = create_features_v8(df, poa_min=poa_min)
    if result[0] is None:
        return None
    ...
    # ── 建模
    NUM_MFS = 7
    print(f"\n建立 ANFIS v8 模型（輸入維度={X_train_scaled.shape[1]}, MF={NUM_MFS}, 輸出=sigmoid）")
    ...
    fig.suptitle(f'ANFIS v8 (Hybrid POA + Panel Calibration)  '
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'anfis_v8_analysis.png')
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    print(f"\n圖表: {plot_path}")
    plt.close()

    # ── 儲存
    model_path  = os.path.join(output_dir, 'anfis_v8.keras')
    scaler_path = os.path.join(output_dir, 'scaler_X_v8.save')
    config_path = os.path.join(output_dir, 'model_config_v8.json')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='ANFIS v8 hybrid POA')
