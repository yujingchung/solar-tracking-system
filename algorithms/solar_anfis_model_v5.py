...
# -*- coding: utf-8 -*-
"""
solar_anfis_model_v5.py
=======================
v5 — Hybrid ANFIS + Physics POA Prior
...
def build_anfis_model(input_dim, num_mfs=7):
    """ANFIS v5：sigmoid 輸出（PR_norm ∈ [0, 1]，物理合理）"""
...
# ════════════════════════════════════════════════════════════════
def create_features_v5(df, poa_min: float = 50.0):
    """
    target = PR_norm = power_W / (theoretical_poa × area × η_STC)
...
def main(file_path=None, output_dir=None, poa_min: float = 50.0,
    ...
    print(f"=== ANFIS v5 訓練啟動 (Hybrid POA + ANFIS) ===")
    print(f"  target = PR_norm = power_W / (theoretical_poa × {PANEL_AREA_M2} × {PANEL_EFF_STC})")
    ...
    # ── 特徵工程
    result = create_features_v5(df, poa_min=poa_min)
    if result[0] is None:
        return None
    ...
    # ── 建模
    NUM_MFS = 7
    print(f"\n建立 ANFIS v5 模型（輸入維度={X_train_scaled.shape[1]}, MF={NUM_MFS}, 輸出=sigmoid）")
    ...
    fig.suptitle(f'ANFIS v5 (Hybrid POA Prior)  '
    ...
    # ── 儲存
    model_path  = os.path.join(output_dir, 'anfis_v5.keras')
    scaler_path = os.path.join(output_dir, 'scaler_X_v5.save')
    config_path = os.path.join(output_dir, 'model_config_v5.json')
